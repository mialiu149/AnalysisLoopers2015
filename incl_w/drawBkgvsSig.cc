...
void drawBkgvsSig( std::string iter = "", float luminosity = 1.0, const string selection = "_inclusive", string type = "ll", string variable = "mll", string region = "passtrig" )
{
  ...
  if(use_ttbar1l) getBackground(  h_ttbar1l, iter, Form("ttbar1l_%s", selection.c_str() ), variable,"lep_onelep" , region );
  if(use_ttbar2l) getBackground(  h_ttbar2l, iter, Form("ttbar2l_%s", selection.c_str() ), variable,"lep_dilep", region );
//  if(use_wjets) getBackground(  h_wjets, iter, Form("wjets_incl_%s", selection.c_str() ), variable, type, region );
  ...
  if(use_zjets) getBackground(  h_zjets, iter, Form("zjets_%s", selection.c_str() ), variable, type, region );
...
//	 h_ttbar->Integral(h_ttbar->FindBin(81),h_ttbar->FindBin(101)-1));
 //        cout<<"Norm factor for Z+jets: "<<norm_factor<<endl;
  if(use_wjets) h_wjets->Scale(1.2*luminosity);
 ...
 if( variable=="mindphi_met_j1_j2")  { xmin = 0;   xmax = 3.2; rebin = 10;  }
...
//drawing on canvas and all that.
  ...
  if( TString(variable).Contains("phi") || variable == "mhtphi" || type    == "em" || variable == "nVert" ||variable == "metphi" || variable == "metphi20" || variable == "metphi40" || variable == "metphi60" || variable == "metphir" ){
	h_wjets->GetYaxis()->SetRangeUser(0, h_wjets->GetMaximum()*1.4 );  
  }
  else{
	h_wjets->GetYaxis()->SetRangeUser(ymin*luminosity, h_wjets->GetMaximum() * ymax );
  }
  if( TString(variable).Contains("phi") || variable == "metphi" || variable == "metphi20" || variable == "metphi40" || variable == "metphi60" || variable == "metphir" ){
	h_wjets->GetYaxis()->SetRangeUser(0, h_wjets->GetMaximum()*1.4 );  
  }
...
//  h_rat->GetXaxis()->SetLimits(xmin,xmax);
  ...
  if( variable == "deltaphi_lep_met" ) h_rat->GetXaxis()->SetTitle("#Delta#phi(l,E_{T}^{miss})");
  if( variable == "mindphi_met_j1_j2" ) h_rat->GetXaxis()->SetTitle("min#Delta#phi(jet,E_{T}^{miss})");
 // if( TString(variable).Contains("phi") ) h_rat->GetXaxis()->SetTitle("E_{T}^{miss} #phi");
  ...
  c1->SaveAs(Form("${plot_output}/h_%s_%s_signalregion_%s_%s_mad.png", variable.c_str(), type.c_str(), selection.c_str(), region.c_str() ));
  c1->SaveAs(Form("${plot_output}/h_%s_%s_signalregion_%s_%s_mad.pdf", variable.c_str(), type.c_str(), selection.c_str(), region.c_str() ));
  return;
}
