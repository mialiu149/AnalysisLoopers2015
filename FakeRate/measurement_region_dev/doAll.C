...
//	  TChain *wj     = new TChain("t"); wj     -> Add(lepton_baby_path + "/WJets_madgraph*.root" ); ScanChain(wj     , "./rate_histos_wj.root"     , myAnalysisID , option);
	  TChain *qcd_el = new TChain("t"); qcd_el -> Add(lepton_baby_path + "/QCD_EM_Enriched*.root"); ScanChain(qcd_el , "./rate_histos_qcd_el.root" , myAnalysisID , option);
	  TChain *qcd_mu = new TChain("t"); qcd_mu -> Add(lepton_baby_path + "/QCD_Mu_Enriched*.root"); ScanChain(qcd_mu , "./rate_histos_qcd_mu.root" , myAnalysisID , option);
