void run_lepbabymaker(TString input_path, TString output_path)
{
...
//    gROOT->ProcessLine(".L vtx_reweight.h");
    gROOT->ProcessLine(".L "+fakeratedir+"/code/ScanChain_LepBabyMaker.C+");
    gROOT->ProcessLine("TString input_path = \""+input_path+"\";");
    gROOT->ProcessLine("TString output_path = \""+output_path+"\";");
    gROOT->ProcessLine("TString ttreename = \"Events\";");
    ...
    gROOT->ProcessLine("ScanChain_LepBabyMaker(chain,output_path);");
}
