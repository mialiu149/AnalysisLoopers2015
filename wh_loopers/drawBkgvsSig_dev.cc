...
using namespace std;
void drawBkgvsSig_dev( std::string iter = "", float luminosity = 1.0, const string selection = "_inclusive", string type = "ll", string variable = "mll", string region = "passtrig" , int sigscaleup = 1.0,  int rebin=10,float xmin = 50, float xmax = 500, bool setlog =false, bool use_data=false,bool use_sig=false, bool drawmoneyplot=false)
{
  bool usetemplates   = false;
  bool drawsys= false;
  if (drawmoneyplot) {setlog = false; drawsys = true;}
  ...
  //if(use_wjets) getBackground(  h_wjets, iter, Form("ws_incl_%s", selection.c_str() ), variable,"lep_LF", region );
...
//	 h_ttbar->Integral(h_ttbar->FindBin(81),h_ttbar->FindBin(101)-1));
 ...
 // normalization for signals.
  ...
  if(!setlog) {ymax = 1.5;}
 int nbins = 7;
 double bins[8] = {30,60,90,120,150,210,270,390};
...
//  h_rat->GetXaxis()->SetLimits(xmin,xmax);
  ...
  TString xTitle = getVariableName(variable);
  h_rat->GetXaxis()->SetTitle(xTitle);
