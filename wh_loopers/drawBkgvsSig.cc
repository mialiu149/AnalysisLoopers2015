...
void drawBkgvsSig( std::string iter = "", float luminosity = 1.0, const string selection = "_inclusive", string type = "ll", string variable = "mll", string region = "passtrig" )
{
 ...
 // normalization for signals.
  ...
  if( TString(variable).Contains("met") || TString(variable).Contains("mt") || TString(variable).Contains("MT2") || TString(variable).Contains("mbb") ||TString(variable).Contains("ptbb"))
   {  rebin = 20; xmin = 0;	  xmax = 500;           ymax = 5e1; }
  ...
  if( TString(variable).Contains("pt") ){	xmin = 0;	xmax = 500;	rebin = 20;  }
...
/*TPad *pad = new TPad( "p_main", "p_main", 0.0, 0.2, 1.0, 1.0);
  ...
  */
  if(use_ttbar2l) { h_ttbar2l->SetFillStyle(1001);h_ttbar2l->SetFillColor(kCyan-3);h_ttbar2l->SetLineColor(kCyan-3) ;updateoverflow( h_ttbar2l, xmax );}
  if(use_ttbar1l) { h_ttbar1l->SetFillStyle(1001);h_ttbar1l->SetFillColor(kCyan-2);h_ttbar1l->SetLineColor(kCyan-2) ;updateoverflow( h_ttbar1l, xmax );}
  if(use_data) { updateoverflow( h_data , xmax );  h_data->SetMarkerStyle(8); h_data->SetMarkerSize(0.75); }
  if(use_top) { h_top->SetFillColor(kOrange-2); updateoverflow( h_top, xmax );h_top->SetLineColor(kOrange-2);}
  ...
  if(use_wbb) { h_wbb->SetLineColor(kAzure-5);updateoverflow( h_wbb, xmax );h_wbb->SetFillColor(kAzure-5);}
  if(use_wjets) { h_wjets->SetFillColor(kAzure); updateoverflow( h_wjets, xmax );h_wjets->SetLineColor(kAzure);}
  if(use_diboson) { h_diboson->SetLineColor(kOrange-3);updateoverflow( h_diboson, xmax );h_diboson->SetFillColor(kOrange-3);}
...
//  c1->SetWindowSize(800, 600);
  ...
  if(use_sig) { 
     h_sig_225_80->SetLineColor(kBlack); h_sig_225_80->SetLineStyle(8);h_sig_225_80->SetLineWidth(4); updateoverflow(h_sig_225_80,xmax);  
     h_sig_250_1->SetLineColor(kPink-2);  h_sig_250_1->SetLineStyle(8); h_sig_250_1->SetLineWidth(4);  updateoverflow(h_sig_250_1,xmax);
     h_sig_300_80->SetLineColor(kViolet+1); h_sig_300_80->SetLineStyle(8);h_sig_300_80->SetLineWidth(4); updateoverflow(h_sig_300_80,xmax);
     h_sig_350_1->SetLineColor(kSpring-7);  h_sig_350_1->SetLineStyle(8);h_sig_350_1->SetLineWidth(4);   updateoverflow(h_sig_350_1,xmax);
