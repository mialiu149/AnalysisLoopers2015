...
//  getBackground(  h_QCD, iter, Form("QCD_%s", selection.c_str() ), variable, type, region );
  ...
  else getBackground(  h_wjets, iter, Form("ttbar_%s", selection.c_str() ), variable, type, region );
...
//	h_wjets->Scale(1./h_wjets->GetSumOfWeights());
  ...
  if( TString(variable).Contains("met") || TString(variable).Contains("mt") || TString(variable).Contains("MT2W") || TString(variable).Contains("mbb")){
	if( type == "em" ) rebin = 40;
	else rebin = 40;
	  xmin = 0;
...
//  l1->AddEntry( h_QCD , "QCD"          , "f");
  ...
  TPad *rat_pad = new TPad( "p_main", "p_main", 0.0, 0.0, 1.0, 0.2);
