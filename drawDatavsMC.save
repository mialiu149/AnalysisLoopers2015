...
using namespace std;
void drawDatavsMC( std::string iter = "", float luminosity = 1.0, const string selection = "_inclusive", string type = "ll", string gentype = "2l",string variable = "mll", string region = "passtrig" )
{
  ...
  getBackgroundGenSplit(   h_data, iter, Form("data%s" , selection.c_str() ), variable, type, region );
  if( usefsbkg ) getBackgroundGenSplit(  h_ttbar2l, "V07-04-03_updatedHLT", Form("data%s", selection.c_str() ), "metgt1jet", "em", "inclusive" );
  else {
  getBackgroundGenSplit(  h_ttbar2l, iter, Form("ttbar%s", selection.c_str() ), variable, type,"2l", region );
  getBackgroundGenSplit(  h_zjets, iter, Form("zjets_htbin%s", selection.c_str() ), variable, type,"2l", region );
  getBackgroundGenSplit(  h_QCD, iter, Form("QCD%s", selection.c_str() ), variable, type,"2l" ,region );
  }
  // getBackgroundGenSplit(  h_wjets, iter, Form("wjets%s", selection.c_str() ), variable, type, region );
  // getBackgroundGenSplit(  h_other, iter, Form("ttv%s"   , selection.c_str() ), "met", "ll" );
  // getBackgroundGenSplit(  h_vvbkg, iter, Form("vv%s"    , selection.c_str() ), "met", "ll" );
  // getBackgroundGenSplit(  h_wjets, iter, "All_MC", "jzb", "ll" );
  if( usetemplates ) getTemplateMET( h_wjets, "V07-04-03_updatedHLT", Form("data%s", selection.c_str() ) );
  else getBackground(  h_wjets, iter, Form("wjets_htbin%s", selection.c_str() ), variable, type, region );
  // getTemplateMET( h_wjets, iter, Form("data%s", selection.c_str() ) );
  // h_wjets->Scale(0.057);
  // h_ttbar2l->Scale(0.057);
...
//	h_wjets->Scale(scaleval);
  ...
  h_ttbar2l->SetFillColor(kCyan-3);
  ...
  float norm_factor =   h_data->Integral(h_data->FindBin(81),h_data->FindBin(100)-1)/
	(h_wjets->Integral(h_wjets->FindBin(81),h_wjets->FindBin(101)-1) +
	 h_ttbar2l->Integral(h_ttbar->FindBin(81),h_ttbar->FindBin(101)-1));
...
//  h_ttbar2l->Scale(luminosity);
  ...
  if( usefsbkg ){
  	l1->AddEntry( h_ttbar2l , "FS Bkg"       , "f");
  }else{
	l1->AddEntry( h_ttbar2l , "t#bar{t} MC"       , "f");
  }
