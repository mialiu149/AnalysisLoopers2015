...
void drawBkgvsSig( std::string iter = "", float luminosity = 1.0, const string selection = "_inclusive", string type = "ll", string variable = "mll", string region = "passtrig" )
{
  ...
  bool usefsbkg       = false;
  bool use_data(true); 
  bool use_wjets(true),use_ttbar(false),use_ttbar1l(true),use_ttbar2l(true),use_zjets(false),use_wbb(false);
  bool use_top(true),use_ttv(true),use_diboson(false);
  bool use_sig(false);
  bool use_norm_factor(false); float norm_factor = 0.75;
  int scaleup = 100;
  ...
  if(use_data)  getBackground(  h_data, iter, Form("data_onelep_%s" , selection.c_str() ), variable, type, region );
